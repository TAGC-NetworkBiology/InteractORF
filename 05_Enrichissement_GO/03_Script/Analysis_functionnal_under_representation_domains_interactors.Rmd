---
title: Analyse sous représentation fonctionnelle des Protéines interagissant avec les domaines
  des sORFs
...
## Sous représentation fonctionnelle des protéines interagissant avec les domaines des sORFs (contre l'interactome entier des monocytes)

L'objectif de cette analyse est d'étudier la sous représentation fonctionnelle des protéines interagissant avec les domaines des sORFs. Pour ce faire, nous avons réalisé un test de sous-representation en termes GO via g:profiler, en comparant les termes GO associés aux interacteurs (domaine-domaine) des sORFs à l'ensemble de l'interactome des monocytes.

Cette analyse vise à répondre à la question biologique suivante : __Y a-t-il une fonction sous représentée parmi les interacteurs des sORFs contenant des domaines au sein des monocytes?__
...
data <- read.csv("/home/slivak/Bureau/Interactome_sORF_monocytes/05_Enrichissement_GO/01_Reference/gProfiler_results/gProfiler_hsapiens_27-02-2024 14-39-55__intersections.csv", header = TRUE)
...
### Analyse des sous représentations en GO termes avec les meilleures p-values
...
best_pval <- ggplot(data_filtered, aes(x = reorder(term_name, negative_log10_of_adjusted_p_value), y = negative_log10_of_adjusted_p_value, fill = intersection_size)) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(x = "GO Name", y = "-log10 pval", fill = "number of occurences") +
  theme(axis.text.y = element_text(hjust = 1, size = 60),
...
```{r, out.width="130%", fig.asp=1.6, echo=FALSE, fig.height=3, fig.width=60}
plot(best_pval)
```


### Analyse des GO slims associés aux termes GO significativement sous représentés dans les interacteurs des sORFs.
...
### Analyse des sous-représentations en terme Kegg Reactome avec les meilleurs pval
...
simplified_REACT <- read.csv("/home/slivak/Bureau/Interactome_sORF_monocytes/05_Enrichissement_GO/05_Output/Sipmified_REAC/simplified_REAC_gProfiler_hsapiens_27-02-2024 14-39-55__intersections.csv", header = TRUE, sep=",")
...
```
### Analyse des sous-representations en terme Wiki pathways avec les meilleurs pval
Les protéines intéragissant avec les domaines des sORFS ne présentent pas de sous-représentation significative en terme WP par rapport aux protéome des monocytes. 

### Analyse de la sous représentation en terme Human proteome ontology avec les meilleurs pval
```{r, echo=FALSE, include=FALSE}
...
## Sous représentation fonctionnelle des protéines interagissant avec les domaines des sORFs (contre l'ensemble des protines intéragissant avec les sORFs)

L'objectif de cette analyse est d'étudier la sous représentation fonctionnelle des protéines interagissant via des interactions domaine-domaine avec les sORFs. Pour ce faire, nous avons réalisé un test de sous représentation en termes GO via g:profiler, en comparant les termes GO associés aux interacteurs liés aux domaines des sORFs à l'ensemble des interacteurs des sORFs (interagissant soit avec les domaines des sORFs soit avec les slim des sORFs).

Cette analyse vise à répondre à la question biologique suivante :
__Y a-t-il une fonction sous représentée parmi les protéines qui interagissent spécifiquement avec les domaines des sORFs parmi tous les interacteurs des sORFs?__
...
data <- read.csv("/home/slivak/Bureau/Interactome_sORF_monocytes/05_Enrichissement_GO/01_Reference/gProfiler_results/gProfiler_hsapiens_27-02-2024 14-43-22__intersections.csv", header = TRUE)
...
### Analyse des sous-représentations en GO termes avec le plus grand nombre d'occurence

```{r, include=FALSE}
data_filtered_occurences <- data %>%
  filter(source %in% c('GO:MF', 'GO:BP', 'GO:CC')) %>%
  group_by(source) %>%
  arrange(intersection_size) %>%
  top_n(30, wt = intersection_size)  # Sélectionne les 30 meilleurs résultats par facet

best_occurences <- ggplot(data_filtered_occurences, aes(x = reorder(term_name, negative_log10_of_adjusted_p_value), y = negative_log10_of_adjusted_p_value, fill = intersection_size)) +
  geom_bar(stat = "identity") +
  scale_fill_gradient(low = "blue", high = "red") +  # Spécifie les limites de la légende
  facet_wrap(~ source, scales = "free_y", nrow = 3) +
  coord_flip() +
...
```{r, out.width="130%", fig.asp=1.7, fig.height=3, fig.width=60, echo=FALSE}
plot(best_occurences)
```
